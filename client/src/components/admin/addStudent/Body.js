...
const Body = () => {
  ...
  if (!/^\d{10}$/.test(value.fatherContactNumber)) {
    setError({ fatherContactError: "Father's contact number must be exactly 10 digits" });
    setLoading(false);
    return;
  }

  if (!/^\d{10}$/.test(value.motherContactNumber)) {
    setError({ motherContactError: "Mother's contact number must be exactly 10 digits" });
    setLoading(false);
    return;
  }
  ...
  onChange={(e) => {
    ...
    if (/^\d*$/.test(input)) {
      setValue({
        ...value,
        fatherContactNumber: input,
      });
      // Clear error if valid
      if (input.length === 10) {
        setError((prev) => ({ ...prev, fatherContactError: "" }));
      }
    }
  }}
/>
{error.fatherContactError && (
  <p className="text-red-500 text-sm">{error.fatherContactError}</p>
)}
  ...
  onChange={(e) => {
    ...
    if (/^\d*$/.test(input)) {
      setValue({
        ...value,
        motherContactNumber: input,
      });
      // Clear error when valid
      if (input.length === 10) {
        setError((prev) => ({ ...prev, motherContactError: "" }));
      }
    }
  }}
/>
{error.motherContactError && (
  <p className="text-red-500 text-sm">{error.motherContactError}</p>
)}
                </div>
                <div className={classes.adminForm3}>
  ...
  <Select
    ...
    disabled={!value.year} // disable when no year is selected
  >
    <MenuItem value="">None</MenuItem>
